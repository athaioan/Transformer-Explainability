...
from expl_hdf5 import ImagenetResults
...
def eval(args):
    ...
    for batch_idx, (data, vis, target) in enumerate(sample_loader):
        ...
        pred_org_logit = pred.data.max(1, keepdim=True)[0].squeeze(1) ## max value
        pred_org_prob = pred_probabilities.data.max(1, keepdim=True)[0].squeeze(1) ## max value probability
        pred_class = pred.data.max(1, keepdim=True)[1].squeeze(1) ## argmax index of class
        tgt_pred = (target == pred_class).type(target.type()).data.cpu().numpy() ## flag True when pred = gt
    ...
    Yall = np.vstack((num_correct_model, num_correct_pertub))
    Y_pertub = num_correct_pertub
    AUC_all = np.trapz(np.mean(Yall, axis=1), dx=0.1)
    AUC_pertub = np.trapz(np.mean(Y_pertub, axis=1), dx=0.1)
    print(AUC_all)
    print(AUC_pertub)
